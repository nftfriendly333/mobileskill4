...
<nav id="site-nav">
  ...
  <div class="sn-links">
    ...
    <a href="reap-what-you-sow-v3.html" class="sn-reap">🌾 Reap What You Sow</a>
    <a href="trade-together.html" class="sn-trade">📊 Trade Together</a>
    <a href="dashboard.html" class="sn-dashboard">◈ Dashboard</a>
    <a href="2nd_Tournament.html" class="sn-tourney">🏆 2nd Tournament</a>
  </div>
</nav>
...
// Phase 1: Render game immediately so it's playable right away
(function() {
  renderSkills();
  resetPlayerStats();
  updateXPDisplay();
...
// Phase 2: Auth + cloud save in background — doesn't block gameplay
(async () => {
  try {
    ...
    // Sync skin to profile
    const _skinEmoji = SKINS[shopState.equippedSkin]?.emoji || '🧙';
    await PveAuth.saveProfile({ skin: _skinEmoji });
    ...
    // Auth failed or cancelled — game still playable, just no cloud save
    console.warn('PvE Auth:', e?.message || e);
  }
})();
...
PveAuth.onLogin(async (profile) => {
  ...
  if (loaded) {
    showSaveToast('✔ Save loaded for new wallet!', '#27ae60');
    claimGuildXP();
  } else {
